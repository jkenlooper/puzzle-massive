...
class HashColorService {
    constructor() {
        ...
        window.addEventListener("pm-hash-color-change", (ev) => {
            ...
            window.location.hash = `${HashColorService.HashBackgroundPrefix.substr(1)}${color}`;
        });
        // Set hash if defined in localstorage and no hash is already set.
        if (this.backgroundColor === undefined) {
            const storedBackgroundColor = window.localStorage.getItem(HashColorService.LocalStorageName + ":" + puzzleNameFromLocation());
    ...
    _onhashchange() {
        if (window.location.hash.startsWith(HashColorService.HashBackgroundPrefix)) {
            const hashBackgroundColor = "#" +
                window.location.hash.substr(HashColorService.HashBackgroundPrefix.length);
            if (this.backgroundColor !== hashBackgroundColor) {
                window.localStorage.setItem(HashColorService.LocalStorageName + ":" + puzzleNameFromLocation(), hashBackgroundColor);
