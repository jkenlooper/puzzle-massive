...
customElements.define(tag, class PmPuzzleImagePicker extends HTMLElement {
    constructor() {
        super();
        this.pageSize = 0;
        ...
        const piecesCountListAttr = this.attributes.getNamedItem("pieces-count-list");
        ...
        this.addEventListener("filterGroupItemValueChange", this._onFilterGroupItemValueChange.bind(this));
    }
    ...
    _setPuzzleImages() {
        ...
        this.render();
        const page = !this.filterPage || !Array.isArray(this.filterPage)
            ? 1
        ...
        const orderby = this.orderBy && this.orderBy.length ? this.orderBy[0] : "m_date";
        ...
        return puzzleImagesService
            .getPuzzleImages(this.filterStatus || [], this.filterType || [], piecesMin, piecesMax, page, orderby)
            .then((puzzleList) => {
            ...
            this.pageCount = Math.ceil(puzzleList.puzzleCount / puzzleList.pageSize);
            this.render();
            return true;
        })
            .then(() => {
...
function getPiecesString(maxPieces, piecesCountList) {
    ...
    if (pieces.length < piecesCountList.length) {
        ...
        pieces.push(piecesCountList[piecesCountList.indexOf(pieces[pieces.length - 1]) + 1]);
    }
    ...
    if (piecesWithDefault.length) {
        ...
        piecesWithDefault[piecesWithDefault.length - 1] = `*${piecesWithDefault[piecesWithDefault.length - 1]}`;
    }
