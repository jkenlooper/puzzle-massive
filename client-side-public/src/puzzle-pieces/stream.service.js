...
import { puzzleStreamMachine, RECONNECT_INTERVAL, } from "./puzzle-stream-machine";
...
class PuzzleStream {
    ...
    getEventSource(puzzleId) {
        ...
        eventSource.addEventListener(EventType.invalid, this.handleInvalidEvent.bind(this), false);
        eventSource.addEventListener(EventType.ping, this.handlePingEvent.bind(this), false);
        eventSource.addEventListener(EventType.move, this.handleMoveEvent.bind(this), false);
        eventSource.addEventListener(EventType.karma, this.handleKarmaEvent.bind(this), false);
        eventSource.addEventListener(EventType.message, this.handleMessageEvent.bind(this), false);
        eventSource.addEventListener(EventType.open, this.handleOpenEvent.bind(this), false);
        eventSource.addEventListener(EventType.error, this.handleErrorEvent.bind(this), false);
        return eventSource;
    }
    get readyState() {
    ...
    destroyEventSource() {
        ...
        this.eventSource.removeEventListener(EventType.invalid, this.handleInvalidEvent, false);
        this.eventSource.removeEventListener(EventType.ping, this.handlePingEvent, false);
        this.eventSource.removeEventListener(EventType.move, this.handleMoveEvent, false);
        this.eventSource.removeEventListener(EventType.move, this.handleKarmaEvent, false);
        this.eventSource.removeEventListener(EventType.message, this.handleMessageEvent, false);
        this.eventSource.removeEventListener(EventType.open, this.handleOpenEvent, false);
        this.eventSource.removeEventListener(EventType.error, this.handleErrorEvent, false);
        ...
        pong
            ...
            if (response) {
                switch (response.name) {
                    case "success":
                        this.broadcast(puzzlePingTopic, response.data && response.data.latency);
                        break;
                    case "ignored":
                        // do nothing
                        break;
...
_a = socketDisconnected, _b = socketConnected, _c = socketReconnecting, _d = pieceUpdate, _e = karmaUpdated, _f = puzzlePingTopic, _g = puzzlePingErrorTopic, _h = puzzleStatusTopic;
