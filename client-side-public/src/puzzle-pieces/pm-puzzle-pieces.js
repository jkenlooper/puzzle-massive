...
customElements.define(tag, class PmPuzzlePieces extends HTMLElement {
    ...
    constructor() {
        ...
        this.blocked = false;
        this.blockedTimer = 0;
        this.piecesPaused = false;
        this.renderedShadowPieces = {};
        const self = this;
        this.instanceId = PmPuzzlePieces._instanceId;
        this.renderPiecesBuffer = [];
        this.innerHTML = html;
        this.$collection = (this.querySelector(".pm-PuzzlePieces-collection"));
        this.$dropZone = (this.querySelector(".pm-PuzzlePieces-dropZone"));
        this.$container = this.querySelector(".pm-PuzzlePieces");
        ...
        const maxPausePiecesTimeout = this.attributes.getNamedItem("max-pause-pieces-timeout");
        this.maxPausePiecesTimeout = parseFloat(maxPausePiecesTimeout ? maxPausePiecesTimeout.value : "0");
        const puzzleStatus = this.attributes.getNamedItem("status");
        this.puzzleStatus = puzzleStatus
            ? parseInt(puzzleStatus.value)
            : undefined;
        ...
        puzzleService
            ...
            const pieceUpdatesService = new FetchService(`/newapi/puzzle-piece-updates/${obj.timestamp}/`);
        ...
        if (this.puzzleStatus &&
            ...
            streamService.subscribe("puzzle/status", this.onPuzzleStatus.bind(this), this.instanceId);
            streamService.subscribe("karma/updated", this.onKarmaUpdated.bind(this), `karmaUpdated ${this.instanceId}`);
            puzzleService.connect();
            puzzleService.subscribe("pieces/mutate", this.batchRenderPieces.bind(this), this.instanceId);
            puzzleService.subscribe("pieces/shadow-mutate", this.renderShadowPieces.bind(this), this.instanceId);
            puzzleService.subscribe("pieces/update", this.renderPieces.bind(this), this.instanceId);
            puzzleService.subscribe("piece/move/blocked", this.onMoveBlocked.bind(this), this.instanceId);
            puzzleService.subscribe("pieces/info/toggle-movable", this.onToggleMovable.bind(this), this.instanceId);
            puzzleService.subscribe("pieces/info/pause-resume", this.onPiecesPauseResume.bind(this), this.instanceId);
        }
        ...
        this.$dropZone.addEventListener("mousedown", this.dropTap.bind(this), false);
        ...
        mc.on("panstart panmove", function (ev) {
            ...
            switch (ev.type) {
                case "panstart":
                    ...
                    break;
                case "panmove":
                    self.$slabMassive.scrollTo(panStartX + ev.deltaX * -1, panStartY + ev.deltaY * -1);
                    break;
            }
        });
        this.$collection.addEventListener("mousedown", this.onTap.bind(this), false);
        hashColorService.subscribe(this.updateForegroundAndBackgroundColors.bind(this), this.instanceId);
    ...
    _pieceFollow(ev) {
        puzzleService.moveBy(this.draggedPieceID, Number(this.$slabMassive.offsetX) +
            ev.pageX -
            this.slabMassiveOffsetLeft, Number(this.$slabMassive.offsetY) +
            ev.pageY -
            this.slabMassiveOffsetTop, this.$slabMassive.scale * this.$slabMassive.zoom);
    }
    stopFollowing(data) {
        data.forEach((pieceMovementData) => {
            ...
            if (pieceMovementData.id === this.draggedPieceID &&
                this.draggedPiece &&
                this.draggedPiece.classList.contains("is-pending")) {
                streamService.unsubscribe("piece/update", `pieceFollow ${this.instanceId}`);
                this.$slabMassive.removeEventListener("mousemove", this.pieceFollow, false);
            }
        });
    }
    dropTap(ev) {
        ...
        if (typeof this.draggedPieceID === "number") {
            puzzleService.unsubscribe("piece/move/rejected", `pieceFollow ${this.draggedPieceID} ${this.instanceId}`);
            puzzleService.dropSelectedPieces(Number(this.$slabMassive.offsetX) +
                ev.pageX -
                this.slabMassiveOffsetLeft, Number(this.$slabMassive.offsetY) +
                ev.pageY -
                this.slabMassiveOffsetTop, this.$slabMassive.scale * this.$slabMassive.zoom);
        ...
        document.addEventListener("keydown", (event) => {
            if (event.key == "Enter") {
                ...
                puzzleService.unsubscribe("piece/move/rejected", `pieceFollow ${this.id} ${this.instanceId}`);
    ...
    onTap(ev) {
        ...
        if (ev.target.classList.contains("p")) {
            ...
            if (ev.target.classList.contains("p") &&
                ...
                puzzleService.subscribe("piece/move/rejected", this.onPieceUpdateWhileSelected.bind(this), `pieceFollow ${id} ${this.instanceId}`);
                // tap on piece
                ...
                this.$slabMassive.addEventListener("mousemove", this.pieceFollow, false);
                // subscribe to piece/update to unfollow if active piece is updated
                streamService.subscribe("piece/update", this.stopFollowing.bind(this), `pieceFollow ${this.instanceId}`);
            }
            else {
                puzzleService.unsubscribe("piece/move/rejected", `pieceFollow ${this.draggedPieceID} ${this.instanceId}`);
                puzzleService.dropSelectedPieces(Number(this.$slabMassive.offsetX) +
    ...
    onPieceUpdateWhileSelected(data) {
        ...
        this.$slabMassive.removeEventListener("mousemove", this.pieceFollow, false);
        // Stop listening for any updates to this piece
        puzzleService.unsubscribe("piece/move/rejected", `pieceFollow ${data.id} ${this.instanceId}`);
        // Just unselect the piece so the next on tap doesn't move it
        puzzleService.unSelectPiece(data.id);
    }
    renderPieceKarma($piece, karmaData) {
        let $_piece = $piece || this.$collection.querySelector("#p-" + karmaData.id);
    ...
    _reducePieces(pieces) {
        if (pieces.length > 1) {
            const pieceProps = {};
            pieces = Object.entries(pieces.reduce((acc, piece) => {
                if (acc[piece.id]) {
    ...
    renderShadowPieces(pieces) {
        let tmp; // = document.createDocumentFragment();
        const unrenderedShadowPieces = [];
        ...
        pieces.forEach((piece) => {
            const pieceID = piece.id;
            let $piece = (this.$collection.querySelector(".s-" + pieceID));
            if (!$piece) {
                if (tmp === undefined) {
                    tmp = document.createDocumentFragment();
                }
                ...
                let $realPiece = (this.$collection.querySelector("#p-" + pieceID));
                if ($realPiece) {
