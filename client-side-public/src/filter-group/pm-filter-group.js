...
(function (FilterGroupType) {
    FilterGroupType["Checkbox"] = "checkbox";
    FilterGroupType["Radio"] = "radio";
    FilterGroupType["Interval"] = "interval";
    FilterGroupType["Pagination"] = "pagination";
})(FilterGroupType || (FilterGroupType = {}));
const filterGroupTypeStrings = [
...
export default class PmFilterGroup extends HTMLElement {
    constructor() {
        ...
        this.isReady = false;
        this._labelList = [];
        this.filtertype = FilterGroupType.Checkbox;
        ...
        if (!nameAttr || !nameAttr.value) {
            throw new Error(`${this.legend} ${this.instanceId} name attribute not set.`);
        }
        else {
        ...
        if (checkedValues !== null) {
            try {
                ...
                // parseable.
                console.warn(`Error: ${err}. Using default value for filter group ${name}`);
            }
        }
        this._valueList = _valueList.map((item) => item.replace(/^\*/, ""));
        if (this.labelList.length &&
            this.labelList.length !== this._valueList.length) {
            throw new Error(`${this.legend} ${this.instanceId} must have same number of labels and values. (${this.labelList}) (${this._valueList})`);
        }
    }
    ...
    set valueList(value) {
        ...
        this._checkedList = storedCheckedList.length
            ? storedCheckedList
            : _checkedValueList;
        window.localStorage.setItem(`${prefix}${this.name}`, JSON.stringify(this._checkedList));
    ...
    handleItemValueChange(e) {
        ...
        window.localStorage.setItem(`${prefix}${this.name}`, JSON.stringify(this._checkedList));
        this.render();
        const filterGroupItemValueChangeEvent = new CustomEvent("filterGroupItemValueChange", {
            detail: {
                name,
                checked,
            },
    ...
    template(data) {
      ...
      <fieldset
        class=${classMap({
            ...
            "pm-FilterGroup": true,
            "pm-FilterGroup--interval": FilterGroupType.Interval === this.filtertype,
            "pm-FilterGroup--checkbox": FilterGroupType.Checkbox === this.filtertype,
      ...
      >
        ...
        ${data.filtertype === FilterGroupType.Pagination
            ? html `
              ...
              <span
                class=${classMap({
                "u-hidden": data.firstCheckedItemIndex === data.filterItems.length - 1,
            })}
              >
                &hellip;
                <button
            ...
            `
            : html `
              ${repeat(data.filterItems, (item) => item.value, (item) => {
                return html `
    ...
    adoptedCallback() { }
    attributeChangedCallback(attr, _oldValue, _newValue) {
