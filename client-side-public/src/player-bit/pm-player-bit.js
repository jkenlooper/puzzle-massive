...
customElements.define(tag, class PmPlayerBit extends HTMLElement {
    constructor() {
        super();
    }
    render() {
        if (this.player) {
            playerBitImgService
                ...
                if (this.player && typeof this.player === "number") {
                    fragment = fragment.replace(`[[${this.player.toString()}]]`, this.player.toString(36));
                    this.style.setProperty("--pm-PlayerBit-color", colorForPlayer(this.player));
