...
from puzzle.constants import COMPLETED
    ...
    def test_patch_request_invalid(self):
        ...
        with self.app.app_context():
            with self.app.test_client() as c:
                ...
                #empty
                data = {}
                rv = c.patch('/puzzle/abc/piece/2/move/', follow_redirects=True, data=data, headers=headers)
                ...
                data = { 'x': -1, 'y': 2 }
                rv = c.patch('/puzzle/abc/piece/1/move/', follow_redirects=True, data=data, headers=headers)
                assert 400 == rv.status_code
    ...
    def test_patch_request_immutable_move(self):
        ...
        with self.app.app_context():
            with self.app.test_client() as c:
                ...
                data = { 'x': 1, 'y': 2 }
                rv = c.patch('/puzzle/abc/piece/12/move/', follow_redirects=True, data=data, headers=headers)
                assert 400 == rv.status_code
    ...
    def test_patch_request_unauthorized(self):
        ...
        with self.app.app_context():
            with self.app.test_client() as c:
                ...
                data = { 'x': 1 }
                rv = c.patch('/puzzle/abc/piece/2/move/', follow_redirects=True, data=data, headers={'token': '1234invalid'})
                assert 403 == rv.status_code
    ...
    def test_notfound(self):
        ...
        with self.app.app_context():
            with self.app.test_client() as c:
                ...
                #not found
                data = { 'x': 1 }
                rv = c.patch('/puzzle/not/piece/2/move/', follow_redirects=True, data=data, headers=headers)
                assert 404 == rv.status_code

                #not found
                data = { 'x': 1 }
                rv = c.patch('/puzzle/abc/piece/not/move/', follow_redirects=True, data=data, headers=headers)
                assert 404 == rv.status_code

                #out of range
                data = { 'x': 1 }
                rv = c.patch('/puzzle/abc404/piece/4/move/', follow_redirects=True, data=data, headers=headers)
                assert 404 == rv.status_code

                #out of range
                data = { 'x': 1 }
                rv = c.patch('/puzzle/abc/piece/404/move/', follow_redirects=True, data=data, headers=headers)
                assert 404 == rv.status_code
    ...
    def test_notallowed(self):
        ...
        with self.app.app_context():
            with self.app.test_client() as c:
                ...
                #method not allowed
                data = { 'x': 1 }
                rv = c.put('/puzzle/abc/piece/1/move/', follow_redirects=True, data=data, headers=headers)
                assert 405 == rv.status_code

                #method not allowed
                data = { 'x': 1 }
                rv = c.post('/puzzle/abc/piece/1/move/', follow_redirects=True, data=data, headers=headers)
                assert 405 == rv.status_code

                #method not allowed
                data = { 'x': 1 }
                rv = c.get('/puzzle/abc/piece/1/move/', follow_redirects=True, data=data, headers=headers)
                assert 405 == rv.status_code
