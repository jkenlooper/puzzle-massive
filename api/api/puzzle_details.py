...
class PuzzleOriginalDetailsView(MethodView):
    ...
    def get_bid_amount(self, cur, puzzleData):
        ...
        bid_amount = 0
        low, high = next(filter(lambda x: x[0] <= puzzleData['pieces'] and x[1] > puzzleData['pieces'] , SKILL_LEVEL_RANGES))
        queue_count_result = cur.execute(fetch_query_string('get-queue-count-for-skill-level.sql'), {'low': low, 'high': high}).fetchone()
    ...
    def patch(self, puzzle_id):
        ...
        if action == 'bump':
            ...
            player_points_result = cur.execute(fetch_query_string('select-minimum-points-for-user.sql'), {'points': bid_amount, 'user': user}).fetchone()
            ...
            cur.execute(fetch_query_string('bump-puzzle-winning-bid-to-bumped-bid.sql'), {'low': low, 'high': high})

            # bump this puzzle to be the winning bid and decrease player dots
            cur.execute(fetch_query_string('bump-puzzle-queue-winning-bid-for-puzzle.sql'), {'puzzle': puzzleData['id']})
            cur.execute(fetch_query_string('decrease-user-points.sql'), {'points': bid_amount, 'user': user})
            db.commit()
