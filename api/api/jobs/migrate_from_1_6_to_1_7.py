...
if __name__ == "__main__":
    ...
    for file_path in (
        ...
        with open(
            os.path.normpath(os.path.join(os.path.dirname(__file__), file_path)), "r"
        ) as f:
    ...
    (results, col_names) = rowify(
        cur.execute(query_all_users_with_icons, {}).fetchall(), cur.description
    )
    ...
    for file_path in ("../queries/insert_initial_bit_expiration_round_2.sql",):
        with open(
            os.path.normpath(os.path.join(os.path.dirname(__file__), file_path)), "r"
        ) as f:
            for statement in f.read().split(";"):
