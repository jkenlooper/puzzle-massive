...
def migrate(db, puzzle):
    def get_offset_of_adjacent_pieces(piece, mask_width, piece_width):
        ...
        for adjacent_piece in d:
            for s in sides:
                try:
                    if (
                        adjacent_piece[s["other_side"][piece_alignment]]
                        == piece[s["side"][piece_alignment]]
                    ):  # always check if the sides match
                        offset = {
                            "x": int(
                                math.ceil(piece_width)
                                * FITS_MAPPING[s["side"][piece_alignment]]["x"]
                            ),
                            "y": int(
                                math.ceil(piece_width)
                                * FITS_MAPPING[s["side"][piece_alignment]]["y"]
                            ),
                        }
    ...
    if result:
        immovableGroup = (
            int(result[0]["parent"]) if result[0]["parent"] else int(result[0]["id"])
        )
...
if __name__ == "__main__":
    ...
    if all_puzzles:
        for puzzle in all_puzzles:
            print(
                "{id} Migrating puzzle: {puzzle_id} with pieces: {pieces} and status: {status}".format(
                    **puzzle
                )
            )
