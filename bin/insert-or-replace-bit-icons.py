...
def add_bit_icons_in_file_system(db):
    ...
    for source_file in glob.glob(os.path.join("media", "bit-icons", "source-*.yaml")):
        ...
        with open(source_file, "r") as f:
            ...
            for bit_icon_file in glob.glob(
                os.path.join(os.path.dirname(source_file), f"64-{group_name}-*.png")
            ):
                name = os.path.basename(bit_icon_file)[len(group_name) + 4 : -4]
                bit_icon_name = "-".join([group_name, name])
